...
A sequence modelling $R$ network uses the latent representation $z$ as input and produces a non-linear transformation of the same size, $R(z) = LSTM(z) + z$, denoted as $\hat{z}$. The LSTM network is composed of two layers and $2^{L-1}$ $H$ hidden units. For causal prediction, we employ a unidirectional LSTM, whereas for non-causal models, we use a bidirectional LSTM followed by a linear layer that combines the two outputs.
