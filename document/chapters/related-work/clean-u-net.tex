...
The baseline is a U-net architecture, which is composed of an encoder, a decoder, and a bottleneck between them. Both encoder and decoder have the same number of layers, D (Depth of the encoder). Encoder and decoder are connected by skip connections.
...
Model dimension is 512. The attention map is masked, which means that during self-attention, each position in the sequence can only attend to positions earlier in the sequence, to ensure causality.
