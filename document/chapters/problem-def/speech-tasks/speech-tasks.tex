...
A wide range of tasks related to speech processing exists. It revolves about manipulating and operating on spoken language signals \cite{mehrish2023}. The main challenge is to address the difficulties introduced in real-world conditions. In real-world conditions, noise and distortions takes place and fighting these conditions is the main goal of speech enhancement. It previously used algorithmic methods while now shifting towards deep learning-based algorithms to improve the speech signal. A related task is speech dereverberation, which removes the reverberant effect cause by enclosed areas.\\
...
There are other speech related tasks which focus on operating on speech instead of manipulating it. Speech recognition translates the spoken speech into text. Speaker recognition and diarization focuses on identifying and tracking speakers in the given signal. Speech embeddings attempt to convert the signal into low-dimensional vector representations, which allows tasks such as speaker verification and sentiment analysis to exist.
