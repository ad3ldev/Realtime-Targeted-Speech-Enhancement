...
This section provide an overview of the initial project structure. The project structure is designed to be modular and extensible, allowing for easy experimentation and development.
...
\subsubsection{PyTorch Lightning}
...
\begin{enumerate}
    ...
    \item \textbf{Plug-ability}: Hydra is designed to be easy to extend and to integrate with other systems. You can develop plugins to extend Hydra's capabilities.
...
\begin{enumerate}
    ...
    \item \textbf{Reproducibility}: Weights \& Biases helps ensure your models are reproducible by automatically capturing all the relevant information needed to replicate a run.
...
A brief overview of the project's folder structure is provided below.

\subsubsection{Code}
\begin{itemize}
    \item \textbf{archs}: for plain Pytorch nn models.
    \item \textbf{data}: for plain Pytorch datasets.
    \item \textbf{loss}: for custom loss functions.
    \item \textbf{models}: pytorch lightning models.
    \item \textbf{metrics}: for custom metrics.
    \item \textbf{utils}: for misc. utilities (LR schedulers, processing functions, logging, callbacks, etc..).
\end{itemize}

\subsubsection{Configuration}
\begin{itemize}
    \item \textbf{data}: for datasets configurations(train/val/test datasets and dataloaders) with its correspongding lightning data module.
    \item \textbf{model}: for pytorch netowrks configuration with its corresponding lightning model.
    \item \textbf{trainer}: for seed, ckpt path, trainer args, callbacks.
    \item \textbf{training}: for optimizer, losses, metrics.
\end{itemize}
