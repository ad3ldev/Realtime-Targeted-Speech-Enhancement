...
Users can activate or deactivate the 'Cancel Noise' switch to start or stop noise cancellation. Additionally, they can adjust the percentage of noise to be removed using the dry/wet slider.
...
Once these settings have been configured, the user needs to click 'Save' to start noise cancellation. Clicking 'Save' toggles its text to 'Edit' and disables other components, as the user is not allowed to modify their values while the app is running, except for the 'Cancel Noise' switch button, which remains enabled.

If the user clicks 'Edit', the components are enabled again, the text toggles back to 'Save', and noise cancellation is halted until the user edits their necessary data and clicks 'Save' again to resume.
