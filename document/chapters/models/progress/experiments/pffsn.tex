...
The speaker embedding module consists of the encoder and bottleneck layers of FFSN followed by a Global Average Pooling layer to obtain a descriptor 
for each subband. These descriptors are then fed to a simple feedforward network.
...
The Adam\cite{kingma2017adam} optimizer was used with a learning rate of $5 \times 10^{-5}$ with Cosine Annealing\cite{loshchilov2017sgdr} and beta values of 0.9 and 0.999. Model exponential moving average (EMA) was used with a decay of 0.999.
We employed a mixture of loss functions, including L1 loss, Multi-Resolution STFT loss\cite{defossez2020}, complex Ration Mask (cRM) loss\cite{hao2023fast}, and TAP loss\cite{zeng2023tap} with weights 1, 0.5, 1, and 0.3 respectively.
