...
def get_file_name_by_user_id(conn, user_id):
    try:
        c = conn.cursor()
        c.execute(
            "SELECT file_name FROM reference_audios WHERE user_id = ?", (user_id,)
        )
        result = c.fetchone()
        if result:
            return result[0]
...
def get_file_path_by_user_id(conn, user_id):
    try:
        c = conn.cursor()
        c.execute(
            "SELECT file_path FROM reference_audios WHERE user_id = ?", (user_id,)
        )
        result = c.fetchone()
        if result:
            return result[0]
...
def get_path_from_file_name(conn, file_name):
    ...
    cur.execute(
        sql, (file_name,)
    )  # Comma is put to make it a tuple with one element (not a string).
    row = cur.fetchone()
