...
class NoiseSuppressionComponent:
    ...
    def create_switch(self):
        '''
        Create a switch to enable or disable noise suppression.
        '''
        font = ("Helvetica", 20)
    ...
    def switch_event(self):
        ...
        if switch_state == "on": 
            if self.enabled_flag == True:
                self.slider.config(state=tk.NORMAL)
            self.dry_set_slider_value.set(self.dry_set_slider_old_value)
            self.current_value_label.configure(text=f'{self.dry_set_slider_old_value}%')
    ...
    def update_switch_font(self, font_size):
        '''
        Update the font size of the switch.
        '''
    ...
    def create_slider(self):
        ...
        # self.left_label.configure(background=cm.bg_color_dark, foreground=cm.fg_color_dark)
        self.left_label.grid(row=0, column=0, sticky=tk.W)

        self.right_label = ttk.Label(self.slider_frame, text="100% (Wet)")
        # self.right_label.configure(background=cm.bg_color_dark, foreground=cm.fg_color_dark)
    ...
    def slider_changed(self, event):
        ...
        self.dry_set_slider_old_value = new_value
