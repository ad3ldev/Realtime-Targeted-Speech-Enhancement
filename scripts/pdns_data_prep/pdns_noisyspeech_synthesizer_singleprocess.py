...
import sys
sys.path.insert(0,'/kaggle/working/Realtime-Targeted-Speech-Enhancement/scripts/pdns_data_prep')
...
def main_gen(params):
    ...
    while file_num <= params['fileindex_end']:
        ...
        for chose_primary, chose_primary_sf in zip(chosen_clean_reverb, chosen_clean_files_reverb):
            ...
            hyphen = '-'
            clean_primary_filenameonly = chose_primary_sf[:-4].split(os.path.sep)[-1]
            ...
            cleanfilename = 'clean_fileid_'+str(file_num)+'_'+clean_primary_filenameonly+'.wav'
